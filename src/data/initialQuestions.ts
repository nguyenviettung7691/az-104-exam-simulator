...
export const initialQuestions: Question[] = [
  choiceQuestion({
    ...
    hint: "Consider least privilege: which role's scope covers ONLY group management without overlapping user administration or security tasks?",
    options: [
  ...
  choiceQuestion({
    ...
    hint: "In a pilot scenario, you need to target a SPECIFIC GROUP for rollout. Which SSPR scope allows group-based eligibility?",
    options: [
  ...
  choiceQuestion({
    ...
    hint: "The requirement is to avoid creating NEW cloud accounts. Which identity type lets external users sign in with their EXISTING home organization credentials?",
    options: [
  ...
  choiceQuestion({
    ...
    hint: "Filter for roles that cover VM operations (start, stop, deploy) WITHOUT including resource group management or RBAC administration rights.",
    options: [
  ...
  choiceQuestion({
    ...
    scenario:
      "An operations engineer is onboarded with Reader at the subscription scope and Contributor on RG-Dev only during a two-week validation window. The engineer attempts several operational tasks: (1) deploy a virtual machine in RG-Dev, (2) view costs in the subscription's cost analysis, (3) modify a policy assignment in RG-Prod, and (4) create a subnet in a virtual network inside RG-Dev but shared with RG-Prod via peering. No deny assignments, custom roles, or PIM elevation are in place.",
    stem: "Which tasks can the engineer successfully complete, and which will fail?",
    subtopic: "Manage access to Azure resources",
    referenceTopic: "Interpret access assignments and scope inheritance",

    hint: "Effective access is additive AND scope-bound; task success depends on both the role AND the resource scope. Test each task: does the engineer's role at that scope permit it? Reader allows VIEW ONLY; Contributor allows CREATE and UPDATE but not policy or RBAC changes. Consider shared resources and access across boundaries.",
    options: [
      option("A", "Tasks 1 and 2 succeed; tasks 3 and 4 fail. Contributor in RG-Dev allows VM deployment. Reader subscription-wide allows cost viewing. RG-Prod policy and cross-RG network changes require broader roles.", "Correct. Task 1 succeeds (VM in RG-Dev = Contributor scope). Task 2 succeeds (cost viewing = Reader scope). Task 3 fails (RG-Prod policy = outside Contributor scope). Task 4 fails (network changes cross RG boundaries and require network roles)."),
      option("B", "All four tasks succeed because Contributor and Reader together cover the entire subscription.", "Incorrect. Reader and Contributor together do not grant policy management (requires specific RBAC role) or network changes across resource-group boundaries."),
      option("C", "Only task 1 succeeds. Reader blocks all write operations even in RG-Dev.", "Incorrect. Contributor at RG-Dev scope overrides Reader for write operations within that group, allowing task 1 and 4 to succeed (if task 4 is a pure resource create)."),
      option("D", "Tasks 1, 2, and 4 succeed; task 3 fails. The engineer has write access everywhere in RG-Dev including network changes.", "Incorrect. While tasks 1, 2, and 4 may seem plausible, the peering relationship means task 4 (subnet creation) spans RG-Prod, which requires explicit RG-Prod access not held by the engineer."),
    ],
    correctOptionId: "A",
    explanation: "Effective access follows scope hierarchy strictly: Contributor allows write operations ONLY in RG-Dev, while Reader is read-only everywhere else. (1) VM deployment in RG-Dev succeeds (Contributor). (2) Cost viewing succeeds (Reader at subscription). (3) Policy changes in RG-Prod fail (outside Contributor scope; no policy role). (4) Subnet changes in peered networks fail if they cross RG boundaries (Contributor scoped to RG-Dev, not RG-Prod). This tests understanding of multi-step operational reasoning, scope boundaries, and why shared resources require explicit cross-scope permissions.",
  }),
  choiceQuestion({
    id: "Q2006",
    domain: "D1",
    ...
    hint: "You need to ADD tags during deployment AND fix existing resources later. Which policy effect can do BOTH through deployment and remediation?",
    options: [
  ...
  choiceQuestion({
    ...
    scenario:
      "A production resource group enters a change freeze on Friday before a long holiday weekend to prevent accidental deletion of critical workloads. However, the operations team must still perform emergency security patching on running VMs if a zero-day is announced, and database backups must continue to run automatically. On Sunday evening, a critical CVE is published; the team needs to patch immediately but cannot wait for a lock removal approval. By Monday, an automated compliance scan attempts to add a required security tag to all resources but fails to apply changes due to lock restrictions.",
    stem: "Which lock minimizes this operational conflict—supporting emergency patching while preventing accidental resource deletion, yet allowing compliance automation to function?",
    subtopic: "Manage Azure subscriptions and governance",
    referenceTopic: "Configure resource locks",

    hint: "Distinguish lock behaviors: ReadOnly blocks BOTH deletes and writes (patching, tagging); CanNotDelete blocks deletes but allows writes. Why does this matter for security patches? For automated compliance tagging? When should locks be paired with other controls?",
    options: [
      option("A", "Apply ReadOnly lock. Establish a separate break-glass process for emergency patching outside the locked RG.", "ReadOnly prevents both deletion AND writes. Emergency patching would fail even with break-glass access within the same RG. This adds operational complexity without solving the tagging failure."),
      option("B", "Apply CanNotDelete lock. This preserves emergency patch capability and allows automated compliance tagging while preventing accidental RG deletion.", "Correct. CanNotDelete allows write operations (VM patching, compliance tag application) while protecting the RG from deletion. This supports both emergency ops and automation within a single control."),
      option("C", "Do not apply a lock; instead, use a Deny policy to prevent resource deletion and rely on policy remediation for tagging.", "Policy is evaluated at deployment time, not at the point of manual deletion. A Deny policy would not prevent an administrator from manually deleting the RG, and policy remediation may not trigger synchronously for emergency scenarios."),
      option("D", "Apply ReadOnly lock with a time-based exception that automatically removes the lock at 2 AM Monday to allow tagging.", "Time-based exceptions are not a native lock feature. Additionally, keeping ReadOnly in place blocks emergency patching, defeating the operational goal. The question is asking which single lock type works, not a workaround."),
    ],
    correctOptionId: "B",
    explanation:
      "CanNotDelete is the optimal lock for this scenario because it blocks RG deletion (the accidental deletion risk) while still permitting all write operations (VM security patching, compliance tag application, automated processes). ReadOnly would prevent emergency patches and compliance automation, creating a false dilemma. This tests understanding of lock behavior nuances, operational trade-offs, and why locking strategy must account for both prevention (accidental deletion) and permission (emergency/automated operations).",
  }),
  choiceQuestion({
    id: "Q2008",
    domain: "D1",
    ...
    hint: "You have 8 subscriptions across multiple regions. At what SCOPE can you apply governance so it flows down to all child subscriptions automatically?",
    options: [
  ...
  choiceQuestion({
    ...
    hint: "The alert must be informational only and NOT block deployments. Which feature sends threshold-based notifications without enforcement?",
    options: [
  ...
  choiceQuestion({
    ...
    company: "Proseware",
    scenario:
      "Proseware operates 200+ Azure VMs and 50+ storage accounts across three subscriptions in a regulated industry. Leadership wants to reduce monthly spend, surface underutilized VM and disk recommendations, and prioritize actions the small operations team can complete before the next billing cycle. The solution must be native Azure (no third-party tools), run-time lightweight, require minimal onboarding, avoid creating manual tracking overhead, and allow filtering recommendations by resource type and cost impact so the team can focus on quick wins. Current environment: VMs are split across Dev, Staging, and Prod. Log Analytics is in use but not yet collecting VM performance baselines.",
    stem: "Which Azure service should you review first to surface cost-saving opportunities that meet these constraints?",
    subtopic: "Manage Azure subscriptions and governance",
    referenceTopic: "Cost analysis, Azure Advisor, and optimization recommendations",

    hint: "Filter by NATIVE service, QUICK onboarding, RECOMMENDATIONS not just metrics. Does the service provide prioritized RIGHTSIZING guidance, or only raw metrics? Can you filter by cost impact without building custom dashboards? Consider: Does Log Analytics WITH Performance Insights equal a recommendation engine?",
    options: [
      option("A", "Azure Advisor Cost recommendations", "Advisor Cost category surfaces rightsizing recommendations for underutilized VMs and disks with estimated savings, filterable by resource type and impact, requiring zero integration overhead."),
      option("B", "Set up Log Analytics performance collection and build custom KQL queries to identify underutilized VMs", "While Log Analytics provides raw metrics, building custom queries adds manual overhead and does not provide prioritized recommendations or cost-impact estimates."),
      option("C", "Use Azure Cost Management detailed analysis to export VM spend trends and correlate with Activity Log events", "Cost Management shows historical spend but does not recommend rightsizing; correlating with Activity Log requires manual analysis and does not prioritize by cost impact."),
      option("D", "Query Azure Resource Graph to list all VMs, then cross-reference with Azure Monitor metrics to identify unused resources", "Resource Graph and Monitor provide raw data but require custom scripting, do not deliver recommendations, and do not estimate savings—creating manual integration overhead that violates the 'lightweight' constraint."),
    ],
    correctOptionId: "A",
    explanation:
      "Azure Advisor's Cost category is purpose-built to surface rightsizing recommendations (including underutilized VMs and disks) with estimated monthly savings, requires no configuration, and allows filtering by resource type and cost impact. This meets all constraints: native, low-overhead, provides prioritized recommendations (not just raw metrics), and delivers quick-win visibility. Log Analytics requires custom queries and does not estimate savings. Cost Management shows spend trends but not recommendations. Resource Graph/Monitor require scripting and do not provide cost-impact guidance. This question tests understanding of when to use RECOMMENDATION services vs. ANALYSIS services, and recognizes that collecting metrics ≠ surfacing actionable recommendations.",
  }),
  choiceQuestion({
    id: "Q2011",
    domain: "D1",
    ...
    hint: "Think about automation—when group membership changes, how can licenses follow automatically without manual work?",
    options: [
  ...
  choiceQuestion({
    ...
    hint: "Which Azure feature can actively BLOCK resource creation attempts at deployment time without affecting existing resources?",
    options: [
  ...
  multiSelectQuestion({
    ...
    hint: "Think about two pieces: First, what identity object simplifies RBAC management? Second, at what scope and with which role should you assign?",
    options: [
  ...
  choiceQuestion({
    ...
    hint: "Analyze each effect: Audit logs violations; Deny blocks deployment; Modify corrects/adds values. You need one effect that BLOCKS public IPs and another that ADDS tags (even to existing resources via remediation). Which pair handles both without manual intervention? Could you use only Deny? Only Modify? Why or why not?",
    options: [
      option("A", "Audit and Modify", "Audit reports noncompliance but does not block public IPs. Modify adds tags but does not prevent public IP creation."),
      option("B", "Deny and Modify", "Deny blocks public IP deployment. Modify can add the Department tag during deployment and bring existing resources into compliance via remediation—two separate policy assignments satisfy both requirements."),
  ...
  multiSelectQuestion({
    ...
    hint: "You need spending ALERTS and RECOMMENDATIONS. Which two services provide each without blocking deployments?",
    options: [
  ...
  yesNoQuestion({
    ...
    hint: "Test your knowledge of RBAC inheritance and permission models. Consider scope inheritance and what the Reader role actually permits.",
    statements: [
  ...
  yesNoQuestion({
    ...
    hint: "Multi-statement validation: Can external users join groups? Can SSPR be scoped? Does licensing follow future members or only existing ones?",
    statements: [
  ...
  yesNoQuestion({
    ...
    hint: "Validate lock and policy capabilities: What does ReadOnly actually block? Can Policy modify metadata? Do Budgets auto-shutdown resources?",
    statements: [
  ...
  choiceQuestion({
    ...
    hint: "At which SCOPE can you affect multiple subscriptions with a single policy assignment WITHOUT affecting other divisions?",
    options: [
  ...
  dragDropQuestion({
    ...
    scenario:
      "Trey Research has deployed a Modify policy to automatically tag every new and existing resource with a Project tag. The policy is now assigned at the management group scope and has been active for a week, but many existing resources (created months ago) are still missing the Project tag. The compliance team wants to know why resources were not auto-tagged immediately and how to bring legacy resources into compliance without deleting and redeploying them.",
    stem: "Arrange these actions in the correct sequence to understand the policy's auto-tagging behavior and then remediate legacy resources. (Note: Policy creation and assignment happen BEFORE this scenario, so they are already complete.)",
    subtopic: "Manage Azure subscriptions and governance",
    referenceTopic: "Implement and manage Azure Policy remediation and compliance timing",

    hint: "Understand the timing: When does Modify apply? Deployment-time only? What about resources that existed before policy assignment? How do you force compliance on those? Sequencing matters: What must exist before you can check compliance or create remediation?",
    availableItems: [
      "Check policy compliance status to confirm existing resources are non-compliant",
      "Create a remediation task to apply the Modify effect to existing non-compliant resources",
      "Wait for the next resource deployment to observe if new resources receive the tag automatically",
    ],
    answerSlots: [
      "First: Validate current state",
      "Second: Fix legacy resources",
      "Third: Confirm forward behavior (optional validation)",
    ],
    correctOrder: [
      "Check policy compliance status to confirm existing resources are non-compliant",
      "Create a remediation task to apply the Modify effect to existing non-compliant resources",
    ...
    explanation:
      "Modify policies apply automatically to NEW deployments but NOT to resources that existed before the policy was assigned. (1) First, verify non-compliance status to understand the scope of legacy resources. (2) Then, create an explicit remediation task to apply Modify retroactively—remediation is NOT automatic and must be triggered. (3) Finally, validate that future deployments receive the tag, confirming forward behavior. This tests understanding of policy timing (deployment-time for new resources), the necessity of explicit remediation for legacy resources, and the importance of compliance verification before and after remediation.",
  }),
  dragDropQuestion({
    id: "Q2021",
    domain: "D1",
    ...
    hint: "The group must exist BEFORE you can add users to it. Role assignment comes AFTER both group and membership exist.",
    availableItems: [
  ...
  choiceQuestion({
    ...
    hint: "Balance cost vs availability: which tier stays ONLINE (not offline like Archive) but costs less than Hot?",
    options: [
  ...
  choiceQuestion({
    ...
    hint: "You need read access to the REPLICA. Which redundancy option includes '-RA-' for read-access?",
    options: [
  ...
  choiceQuestion({
    ...
    hint: "Service endpoints vs private endpoints: Which keeps the PUBLIC endpoint in use but controls ACCESS by subnet?",
    options: [
  ...
  choiceQuestion({
    ...
    hint: "For revocation without key rotation, bind the SAS to what SERVER-SIDE control that can be modified independently?",
    options: [
  ...
  choiceQuestion({
    ...
    hint: "Large scripted transfers with resume capability: which tool is built for unattended, high-throughput Azure Storage data movement?",
    options: [
  ...
  choiceQuestion({
    ...
    hint: "On-premises domain users need EXISTING credentials over SMB: which authentication method extends their identity into Azure Files?",
    options: [
  ...
  choiceQuestion({
    ...
    hint: "Object replication requires version tracking BEFORE setup: what server-side feature preserves blob history for replication?",
    options: [
  ...
  choiceQuestion({
    ...
    hint: "Customer-managed encryption keys must be stored in which dedicated Azure service? What keeps key ownership separate from alerting/networking?",
    options: [
  ...
  choiceQuestion({
    ...
    hint: "Accidental overwrites need quick recovery without full account restore: what blob feature automatically keeps previous versions?",
    options: [
  ...
  multiSelectQuestion({
    ...
    hint: "Two-step automation: Auto-move to Cool after 30 days AND delete versions after 180 days. Which two features provide both tiering actions and version cleanup?",
    options: [
  ...
  multiSelectQuestion({
    ...
    hint: "Keep public endpoint but restrict to ONE subnet only: which service endpoint feature pairs with firewall VNET rules?",
    options: [
  ...
  multiSelectQuestion({
    ...
    hint: "Two Microsoft tools for interactive AND scripted Azure Storage uploads: which provide both UI and CLI support?",
    options: [
  ...
  yesNoQuestion({
    ...
    hint: "Policy-based revocation vs regional redundancy vs blob recovery: distinguish access control from resiliency and recovery mechanisms.",
    statements: [
  ...
  dragDropQuestion({
    ...
    hint: "Revocable SAS tokens: What policy must exist FIRST, then be referenced in the token, then distributed to the app?",
    availableItems: [
  ...
  choiceQuestion({
    ...
    hint: "Zones in primary region PLUS secondary region replication: which redundancy option combines zonal protection with geo-replication?",
    options: [
  ...
  choiceQuestion({
    ...
    hint: "Concise declarative language that compiles to ARM JSON: what is Azure's domain-specific IaC language?",
    options: [
  ...
  choiceQuestion({
    ...
    hint: "Starting point from existing infrastructure: which portal feature exports deployed resources as a template?",
    options: [
  ...
  choiceQuestion({
    ...
    hint: "Datacenter-level failure protection within ONE region: which availability construct provides zonal separation?",
    options: [
  ...
  choiceQuestion({
    ...
    hint: "VM dependencies must move together: which resources (NICs, disks) need to travel WITH the VM?",
    options: [
  ...
  choiceQuestion({
    ...
    hint: "Encrypt temp disk AND host cache at the HOST level (not just guest OS): which encryption feature covers both?",
    options: [
  ...
  choiceQuestion({
    ...
    hint: "High, predictable performance for SQL Server (not Ultra-class latency requirement): which disk type is standard for production VMs?",
    options: [
  ...
  choiceQuestion({
    ...
    hint: "Identical instances with automatic scaling 2-10 based on CPU: which compute resource provides both uniform deployment and autoscale?",
    options: [
  ...
  choiceQuestion({
    ...
    hint: "30-minute daily batch, no server management, no idle charges: which serverless container service runs once and stops?",
    options: [
  ...
  choiceQuestion({
    ...
    hint: "HTTP microservice with revisions, autoscale, scale-to-zero, managed platform: which service provides ALL these?",
    options: [
  ...
  choiceQuestion({
    ...
    hint: "Private container image storage in Azure: what is the purpose-built service for managing your organization's images?",
    options: [
  ...
  choiceQuestion({
    ...
    hint: "Map subdomain to App Service: which DNS record type creates an alias to the app's default hostname?",
    options: [
  ...
  choiceQuestion({
    ...
    hint: "Staged validation + low-downtime traffic swap: which App Service feature enables blue-green releases?",
    options: [
  ...
  choiceQuestion({
    ...
    hint: "Minimum tier supporting autoscale (not Free/Shared/Basic): which tier unlocks scaling?",
    options: [
  ...
  multiSelectQuestion({
    ...
    hint: "Bicep facts: Does it compile to ARM JSON? Does it support resource group deployments? Or does it replace RBAC/KQL?",
    options: [
  ...
  multiSelectQuestion({
    ...
    hint: "Passwordless container auth: managed identity + AcrPull role for pulling images from private ACR.",
    options: [
  ...
  multiSelectQuestion({
    ...
    hint: "Safe release pattern: isolate new version BEFORE production → test it → swap when ready. Which slot + which action?",
    options: [
  ...
  multiSelectQuestion({
    ...
    hint: "App Service backup prerequisites: plan tier must SUPPORT backups + external storage to HOLD backups. Which two?",
    options: [
  ...
  yesNoQuestion({
    ...
    hint: "Compute services: VM scale sets can auto-scale; ACI works without requiring ACR; deployment slots reduce downtime.",
    statements: [
  ...
  yesNoQuestion({
    ...
    hint: "HA & encryption: zones protect datacenters; encryption at host covers temp disk/cache; ACR stores both Linux AND Windows.",
    statements: [
  ...
  dragDropQuestion({
    ...
    hint: "Bicep workflow: Target must EXIST first → VALIDATE before deploying → DEPLOY → REVIEW. What is the correct sequence?",
    availableItems: [
  ...
  dragDropQuestion({
    ...
    hint: "Blue-green release: First create slot BEFORE deploying → deploy to isolated slot → swap ONLY after validation.",
    availableItems: [
  ...
  choiceQuestion({
    ...
    hint: "Enforce TLS 1.2 minimum: which App Service setting controls minimum accepted TLS protocol version?",
    options: [
  ...
  choiceQuestion({
    ...
    hint: "Private IP connectivity between two VNets over Azure backbone: which VNet-to-VNet feature enables this?",
    options: [
  ...
  choiceQuestion({
    ...
    hint: "Force traffic through firewall appliance: what routing feature defines next-hop as a virtual appliance?",
    options: [
  ...
  choiceQuestion({
    ...
    hint: "NSG troubleshooting: what diagnostic view shows the FINAL merged NSG behavior on a NIC?",
    options: [
  ...
  choiceQuestion({
    ...
    hint: "Secure admin access without public IPs on VMs: which managed service provides browser-based RDP/SSH?",
    options: [
  ...
  choiceQuestion({
    ...
    hint: "Keep PUBLIC endpoint, enable private path from subnet: what extends VNet identity to PaaS over Azure backbone?",
    options: [
  ...
  choiceQuestion({
    ...
    hint: "Private IP-only access to PaaS (no public path): what creates a private IP endpoint inside your VNet?",
    options: [
  ...
  choiceQuestion({
    ...
    hint: "Host authoritative public DNS zone in Azure: which Microsoft service provides DNS zone hosting?",
    options: [
  ...
  choiceQuestion({
    ...
    hint: "Internet-facing L4 traffic distribution to VMs: which load balancer SKU supports public frontend IP?",
    options: [
  ...
  choiceQuestion({
    ...
    hint: "Private load balancing for internal users only: which LB keeps frontend IP private and non-internet-facing?",
    options: [
  ...
  choiceQuestion({
    ...
    hint: "Unhealthy backend after NSG tightening: health probes originate from which Azure service that must be allowed?",
    options: [
  ...
  choiceQuestion({
    ...
    hint: "Standard LB with persistent IP: which SKU + allocation type keeps the same address?",
    options: [
  ...
  multiSelectQuestion({
    ...
    hint: "Private endpoint DNS: create zone HOLDING records + link zone TO VNet. Which two?",
    options: [
  ...
  multiSelectQuestion({
    ...
    hint: "Reduce NSG duplication: use ASGs to target groups of NICs, use service tags for Azure services. Which two?",
    options: [
  ...
  multiSelectQuestion({
    ...
    hint: "UDR effectiveness: route table must EXIST + route table must be ASSOCIATED to subnet. Which two?",
    options: [
  ...
  yesNoQuestion({
    ...
    hint: "VNet peering enables private IP connectivity; Bastion removes public IP requirement; UDRs direct traffic to appliances.",
    statements: [
  ...
  yesNoQuestion({
    ...
    hint: "Private endpoints assign private IP; service endpoints preserve public endpoint; blocked probes cause unhealthy state.",
    statements: [
  ...
  dragDropQuestion({
    ...
    hint: "Load balancer setup sequence: frontend IP → backend resources → health detection → traffic routing. Correct order?",
    availableItems: [
  ...
  dragDropQuestion({
    ...
    hint: "Troubleshoot from intent to evidence: routes first, then NSG effect, then active connectivity diagnostics.",
    availableItems: [
  ...
  choiceQuestion({
    ...
    hint: "A user-defined route to an NVA/firewall VM uses a specific next hop type for virtual appliances.",
    options: [
  ...
  choiceQuestion({
    ...
    hint: "CPU threshold over time is a platform metric condition, not a cost event or control-plane activity log event.",
    options: [
  ...
  choiceQuestion({
    ...
    hint: "For centralized KQL queries across Azure Monitor logs, choose the dedicated log analytics data store.",
    options: [
  ...
  choiceQuestion({
    ...
    hint: "You need to suppress notifications during maintenance without changing alert detection logic.",
    options: [
  ...
  choiceQuestion({
    ...
    hint: "Pick the curated VM monitoring experience that includes performance and dependency maps.",
    options: [
  ...
  choiceQuestion({
    ...
    hint: "Need recurring endpoint reachability tests with historical trend data, not one-time packet capture.",
    options: [
  ...
  choiceQuestion({
    ...
    hint: "Azure VM backup onboarding is vault-first: create the vault before policy and protection steps.",
    options: [
  ...
  choiceQuestion({
    ...
    hint: "Regional VM failover with replication and orchestrated recovery points to the dedicated DR service.",
    options: [
  ...
  multiSelectQuestion({
    ...
    hint: "Split detection from response: one component fires the alert, another handles email/webhook/automation actions.",
    options: [
  ...
  multiSelectQuestion({
    ...
    hint: "For Site Recovery, establish the vault control plane and enable VM replication before any failover action.",
    options: [
  ...
  yesNoQuestion({
    ...
    hint: "Map each tool to its purpose: metrics thresholds, log storage/query, and network connectivity monitoring are different capabilities.",
    statements: [
  ...
  yesNoQuestion({
    ...
    hint: "Differentiate vault usage and prerequisites: Site Recovery vs Backup vault workloads vs VM backup requirements.",
    statements: [
  ...
  yesNoQuestion({
    ...
    hint: "Test failover validates DR safely; processing rules affect notification handling, not alert query conditions.",
    statements: [
  ...
  dragDropQuestion({
    ...
    hint: "Backup onboarding sequence is strict: foundation resource, then policy, then protect workload.",
    availableItems: [
  ...
  choiceQuestion({
    ...
    hint: "Choose least privilege for policy administration and exemptions without granting RBAC role assignment rights.",
  ...
  choiceQuestion({
    ...
    hint: "Need geo-redundancy plus read access to the secondary endpoint during drills.",
    caseStudyId: "CS-CONTOSO-RETAIL",
    options: [
  ...
  choiceQuestion({
    ...
    hint: "Requirement is identical VM instances with native autoscale and low administration overhead.",
    caseStudyId: "CS-CONTOSO-RETAIL",
    options: [
      option("A", "An availability set", "Availability sets improve resiliency for a fixed set of VMs but do not provide built-in autoscale."),
  ...
  choiceQuestion({
    ...
    hint: "Private endpoint DNS needs the right private zone and VNet links so FQDN resolves to private IP.",
    caseStudyId: "CS-CONTOSO-RETAIL",
    options: [
  ...
  choiceQuestion({
    ...
    hint: "If the requirement is orchestrated regional VM failover with replication, focus on DR service capabilities.",
    caseStudyId: "CS-CONTOSO-RETAIL",
    options: [
  ...
  choiceQuestion({
    ...
    hint: "Role must cover guest user lifecycle and group membership management without subscription admin rights.",
  ...
  choiceQuestion({
    ...
    hint: "SMB sign-in must use existing on-prem AD DS credentials, not shared keys or anonymous access.",
    caseStudyId: "CS-FABRIKAM-HEALTH",
    options: [
  ...
  choiceQuestion({
    ...
    hint: "Look for managed HTTP ingress, revisions, and scale-to-zero without Kubernetes cluster operations.",
    caseStudyId: "CS-FABRIKAM-HEALTH",
    options: [
  ...
  choiceQuestion({
    ...
    hint: "Requirement says private IP only and no public exposure for a PaaS service from a subnet.",
    caseStudyId: "CS-FABRIKAM-HEALTH",
    options: [
  ...
  choiceQuestion({
    ...
    hint: "Need one central KQL-capable store for operational log analysis and alerting workflows.",
    caseStudyId: "CS-FABRIKAM-HEALTH",
    options: [
