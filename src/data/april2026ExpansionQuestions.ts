...
export const april2026ExpansionQuestions: Question[] = [
  ...
  choiceQuestion({
    ...
    scenario:
      "Fabrikam Finance's production subscription has a permanent Owner assignment on a critical trading platform. Security policy requires: (1) JIT access—Owner must be eligible, never permanent; (2) activation requires approval from a designated security manager; (3) max activation duration is 8 hours; (4) all activations must be audited with justification recorded in compliance logs; (5) emergency access for production incidents must bypass approval BUT auto-escalate to CISO within 1 hour with incident justification mandatory; (6) failed approval attempts must be locked out from re-requesting for 30 minutes to prevent approval-spam attacks. Additionally, Fabrikam's change-freeze policy blocks all non-emergency activations Dec 20-27. Which combination of configuration steps satisfies all six requirements?",
    stem: "Which approach enforces JIT Owner access with approval gating, audit trails, emergency override with escalation, anti-spam lockout, and change-freeze compliance?",
    subtopic: "Manage access to Azure resources",
    referenceTopic: "Microsoft Entra PIM with multi-layer compliance and emergency workflows",

    hint: "PIM provides eligible assignments, approval workflows, audit logging, and max-duration controls (requirements 1-4). But PIM alone does NOT support: (a) emergency-override-triggered CISO escalation, (b) 30-minute re-request lockout after failed approvals, (c) automatic change-freeze denial based on calendar dates. Which Azure services or policies must you layer to enforce (a), (b), (c)? How would you implement the escalation and lockout logic?",
    options: [
      option("A", "Convert to eligible role in PIM, require approval, set 8-hour max duration, enable PIM audit logs. Enable Conditional Access policy to block activations Dec 20-27. This satisfies all six requirements.", "Incorrect. PIM audit logs satisfy requirement 4. Conditional Access blocks sign-in, not role activation—it cannot enforce activation-time approval gates or emergency override. PIM has no built-in emergency override with escalation (requirement 5) or re-request lockout (requirement 6)."),
      option("B", "Make Owner eligible in PIM, approval required, 8-hour max duration, audit enabled, AND layer Azure Policy with Deny effect for change-freeze dates (Dec 20-27). Create a separate incident-escalation workflow (ServiceNow CRQ or similar) that integrates with PIM: emergency activation requests are auto-logged as high-priority CRQs requiring CISO sign-off within 1 hour. Implement a PIM custom extension or Azure Function to throttle re-requests: after a failed approval, lock out that user from requesting again for 30 minutes.", "Correct. This layering satisfies all six: (1-4) PIM config, (5) external CRQ escalation to CISO within 1-hour SLA, (6) Azure Function-based throttling enforces 30-minute lockout. The policy-based change-freeze denial ensures non-emergency activations are blocked Dec 20-27 while emergency overrides route to CISO (satisfying requirement 5's escalation).\n"),
      option("C", "Configure PIM with two-approver requirement, 8-hour max duration, and enable a 'emergency override' setting in PIM which auto-denies during change-freeze and escalates to CISO. Enable audit logs.", "Incorrect. PIM does not have a built-in 'emergency override' setting or automatic CISO escalation. PIM has no change-freeze calendar-based enforcement. Layering is required for requirements 5 and 6.\n"),
      option("D", "Make Owner eligible, single approver, 4-hour max duration, audit on, Azure Policy Deny for change-freeze, and implement lockout via Conditional Access MFA re-prompt.", "Incorrect. Single approver violates enterprise approval requirements. MFA re-prompt is sign-in control, not request-lockout control. Four-hour max duration conflicts with requirement 3 (8 hours). Azure Policy Deny satisfies change-freeze, but Conditional Access MFA does not throttle PIM re-requests.\n"),
    ],
    correctOptionId: "B",
    explanation:
      "This tests understanding that PIM provides the foundation (eligible + approval + audit + max duration) but CANNOT enforce: (a) incident-triggered escalation to CISO with time-bound SLA, (b) automatic lockout of users after failed approval attempts, (c) calendar-based change-freeze denial. These require layered solutions: external CRQ/incident workflow (requirement 5), Azure Function or custom extension for throttling (requirement 6), and Azure Policy (change-freeze). Option A incorrectly assumes Conditional Access can gate role activation. Option C assumes PIM has emergency-override-with-escalation built in. Option D uses MFA to throttle requests, which is incorrect. The correct answer demonstrates the boundary between PIM (identity/access) and external systems (incident workflow, custom throttling).",
  }),
  choiceQuestion({
    id: "Q2408",
    domain: "D1",
    ...
    company: "Woodgrove",
    scenario:
      "Woodgrove plans a phased Self-Service Password Reset (SSPR) rollout. Pilot phase: selected internal users (marketing team, 50 users) validate SSPR for 30 days. Phase 2: all employees. Phase 3: extend to B2B guest contractors. Requirements: (1) Pilot users must use MFA for SSPR verification (not just email/phone; requires Microsoft Authenticator), (2) Pilot SSPR resets must be logged to Azure Audit for SOX compliance with timestamps and user identity, (3) During pilot, if SSPR fails, users must not fall back to help-desk password resets (they must resolve MFA or contact identity team only), (4) SSPR registration must be mandatory for pilot users before Dec 1 (deadline), and non-compliant users are blocked from SSPR access, (5) At end of pilot, metrics must auto-populate: % users who registered, % password-reset success rate, % failed MFA attempts by user, so leadership can approve Phase 2 rollout, (6) Change-freeze policy Dec 20-27 must block any SSPR configuration changes (no scope additions, no auth method changes during freeze). Which combination of SSPR + Entra + Policy settings satisfies all six requirements?",
    stem: "Which layered configuration provides pilot SSPR with mandatory MFA, SOX audit logging, fallback prevention, registration deadline enforcement, metrics collection, and change-freeze compliance?",
    subtopic: "Manage Microsoft Entra users and groups",
    referenceTopic: "SSPR configuration, MFA requirements, Audit logging, registration policies, policy-based change freeze, metrics/reporting",

    hint: "SSPR scope settings control WHO has access (selected groups = pilot group OK). But SSPR settings don't include: (a) mandatory MFA for SSPR verification (need to configure authentication method policies), (b) audit logging (requires Diagnostic Settings), (c) fallback prevention (need to disable help-desk reset policy or use Policy Deny), (d) registration deadline enforcement (SSPR has no deadline feature; you need policy or custom enforcement), (e) metrics collection (SSPR has limited built-in reporting; you may need Log Analytics queries or custom dashboard), (f) change-freeze compliance (SSPR configs can change; you need policy to gate SSPR setting changes Dec 20-27). Which Azure services layer to achieve all six?",
    options: [
      option("A", "Select 'Selected groups' scope and assign marketing pilot group. Configure SSPR to use MFA (Authenticator app) and enable audit logs. This meets all requirements.", "Incorrect. SSPR scope (selected groups) is correct for pilot access. MFA method selection and audit logs address requirements 1-2. But SSPR settings alone do NOT: prevent help-desk fallback (req 3), enforce registration deadline (req 4), auto-populate metrics (req 5), or prevent configuration changes during freeze (req 6). This is incomplete."),
      option("B", "Configure SSPR for 'Selected groups' (marketing pilot). Enable MFA authentication (Authenticator). Configure Audit Logging via Diagnostic Settings. Disable self-service password reset option in User Settings to force help-desk only (fallback prevention). Use Conditional Access policy to require SSPR registration before Dec 1 with registration enforcement. Create a Policy assignment that blocks Microsoft.Web/sites/* (SSPR config changes) with Deny effect during Dec 20-27. Export SSPR audit logs to Log Analytics and create custom dashboard queries for metrics (registration %, success rate, failed MFA count).", "Correct. This layers multiple services: SSPR selected-groups scope (1), MFA authentication method (1), Diagnostic Settings for audit logs (2), disabled help-desk fallback (3), Conditional Access registration policy with deadline (4), custom Log Analytics dashboards for metrics (5), and Policy Deny for change-freeze (6). Each requirement is addressed by a specific service/setting. This demonstrates that SSPR alone is insufficient; you must layer authentication policies, audit infrastructure, access restrictions, deadlines, metrics, and change controls."),
      option("C", "Configure SSPR 'Selected groups' with MFA. Require all users to register for MFA on their first sign-in (via Conditional Access). Automatically deny all help-desk password resets tenant-wide.",  "Incorrect. First-sign-in MFA registration is broader than SSPR-specific MFA requirement. Denying all help-desk resets tenant-wide (not just pilot phase) disrupts support operations for non-pilot users. This over-applies restrictions."),
      option("D", "Select 'All users' scope for SSPR (to avoid complexity of selective groups). Require MFA. Send all events to audit.", "Incorrect. Selecting 'All users' violates the phased rollout requirement. Pilot validation requires limiting to marketing group only. Requirement 6 (change-freeze compliance) is not addressed."),
    ],
    correctOptionId: "B",
    explanation: "This tests understanding that phased SSPR rollout requires multi-layer controls: SSPR scope (selected groups), MFA authentication methods, audit infrastructure (Diagnostic Settings), fallback prevention (User Settings), registration deadline + enforcement (Conditional Access), metrics visibility (Log Analytics), and change-freeze compliance (Azure Policy). Option A assumes SSPR alone is sufficient. Option C over-applies MFA and help-desk restrictions. Option D violates phased rollout. The correct answer demonstrates that enterprise SSPR governance requires orchestration across 5+ services to achieve security (MFA, audit), usability (deadline, metrics), and compliance (change-freeze) goals.",
  }),
  choiceQuestion({
    id: "Q2409",
    domain: "D1",
  ...
  multiSelectQuestion({
    ...
    scenario:
      "Contoso's support operations team must onboard B2B guest users for a partner portal and maintain membership in access groups tied to applications. Security explicitly forbids this team from Azure subscription/resource administration and from broad tenant-wide privileged roles. The role set should be least privilege for invite + group lifecycle tasks only.",
    stem: "Which TWO Entra roles satisfy these requirements with least privilege?",
    subtopic: "Manage Microsoft Entra users and groups",
    referenceTopic: "Built-in Entra roles",

    hint: "Separate Microsoft Entra directory administration from Azure RBAC resource administration. Choose roles scoped to guest invitation and group administration only.",
    options: [
      option("A", "Guest Inviter", "Correct. Guest Inviter enables invitation of external users without broader Azure resource control."),
      option("B", "Groups Administrator", "Correct. Groups Administrator supports group creation and membership management for application access governance."),
      option("C", "Owner", "Incorrect. Owner is Azure RBAC with broad subscription-level resource control, violating separation of duties."),
      option("D", "Billing Reader", "Incorrect. Billing Reader is cost-visibility focused and does not provide guest onboarding or group membership administration."),
    ],
    selectCount: 2,
    correctOptionIds: ["A", "B"],
    explanation:
      "Guest Inviter plus Groups Administrator provides the exact directory capabilities needed: onboarding external users and administering group memberships. They avoid broad Azure resource permissions that come with roles like Owner. This tests the Entra-role versus Azure-RBAC boundary and least-privilege role composition.",
  }),
  yesNoQuestion({
    id: "Q2412",
    domain: "D1",
  ...
  choiceQuestion({
    ...
    company: "Proseware",
    scenario:
      "Proseware has hierarchy Root > Corp > Prod and Root > Corp > NonProd. A security baseline policy must apply to every current and future production subscription, but must never affect NonProd due to active experimentation workloads. Governance requires no manual per-subscription assignments and no temporary exclusions.",
    stem: "At what scope should you assign the policy to meet these constraints?",
    subtopic: "Manage Azure subscriptions and governance",
    referenceTopic: "Policy assignment scopes",

    hint: "Choose a scope that guarantees inheritance to all prod children while naturally excluding nonprod siblings without exemptions.",
    options: [
      option("A", "Tenant root group", "Incorrect. Root-level assignment would impact both Prod and NonProd unless additional exemptions are introduced."),
      option("B", "Production management group", "Correct. Management-group assignment at Prod inherits to all current/future prod subscriptions and excludes NonProd sibling hierarchy."),
      option("C", "One production resource group", "Incorrect. This is too narrow and fails the requirement for all current/future production subscriptions."),
      option("D", "Single production VM", "Incorrect. VM-level assignment cannot provide subscription-wide or future-inheritance governance."),
    ],
    correctOptionId: "B",
    explanation:
      "Assigning at the Prod management group provides the correct inheritance boundary: broad enough for all production subscriptions (including future ones) and naturally isolated from NonProd siblings. Higher scopes over-apply; lower scopes under-apply and increase operational overhead.",
  }),
  choiceQuestion({
    id: "Q2415",
    domain: "D1",
