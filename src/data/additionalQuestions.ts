...
export const additionalQuestions: Question[] = [
  ...
  choiceQuestion({
    ...
    scenario:
      "Graphic Design Institute has a Corp-Prod management group with subscriptions Split-A and Split-B. Subscriptions Split-A and Split-B have conflicting compliance requirements: Split-A requires all storage-accessing roles to include a denial of blob write operations, while Split-B requires roles to explicitly allow blob writes for regulatory reporting. A custom role must be assignable in BOTH subscriptions, auto-audit all assignments for compliance verification, and prevent accidental role elevation. The role scope must be centrally managed to prevent role creep.",
    stem: "At which scope should you create the custom role definition so it can be centrally managed, assigned in both subscriptions, yet allow split compliance configurations without role duplication?",
    subtopic: "Manage access to Azure resources",
    referenceTopic: "Create and manage custom Azure RBAC roles across management groups",

    hint: "Custom roles created at a management group are inherited by child subscriptions. But what if those subscriptions have CONFLICTING requirements for the same role? At which scope do you CREATE the role, and which tool(s) must overlay to enforce split compliance? Consider: Role SCOPE vs Policy SCOPE for compliance enforcement. Which scope controls ROLE DEFINITION accessibility, and which controls ROLE ASSIGNMENT permissions?",
    options: [
      option("A", "At the Corp-Prod management group. Use Azure Policy (Deny effect) at Split-A to block blob writes, and Modify effect at Split-B to enforce explicit write allowance.", "Correct. The custom role definition lives at Corp-Prod for reusability. Policy assignments at subscription scope layer the compliance constraints: Split-A gets a Deny policy, Split-B gets a Modify policy. This allows one role definition to satisfy conflicting compliance needs."),
      option("B", "Create two separate roles (one for Split-A, one for Split-B) at the subscription scope to ensure each reflects its compliance requirement natively.", "Incorrect. Subscription-scoped roles are not reusable across subscriptions, creating role duplication and audit/governance overhead. The scenario explicitly asks for centrally managed role scope."),
      option("C", "At the Corp-Prod management group. Create the role with BOTH blob-deny AND blob-allow permissions, then use Conditional Access to pick which permission applies per subscription.", "Incorrect. Conditional Access policies apply to sign-in and don't layer role permissions. A single role cannot have contradictory permissions; the conflict must be resolved at the POLICY layer, not the role definition layer."),
      option("D", "At the resource group level within Split-A only, then copy the definition to Split-B at its resource group level.", "Incorrect. Resource-group-scoped role definitions are not portable across subscriptions. The question requires central management and reusability across subscriptions."),
    ],
    correctOptionId: "A",
    explanation:
      "Creating a custom role at the management group scope makes it available and centrally defined for all child subscriptions. The conflicting compliance requirements (Split-A denies blob writes, Split-B enforces writes) are handled by layering Policy assignments at the subscription level, not by creating separate roles. This tests understanding of the separation of concerns: ROLES define permissions; POLICIES layer compliance constraints on top. By combining role scope (management group) with policy scope (subscription), you achieve central role management while supporting split compliance requirements without duplication.",
  }),
  choiceQuestion({
    id: "Q2105",
    domain: "D1",
    ...
    scenario:
      "Southridge Video's Audit management group contains three compliance-sensitive subscriptions: Prod-Audit (PROD), Test-Audit (TEST), and Archive-Audit (ARCHIVED). The audit team must have inherited read-only access across all three. However, compliance requirements differ: Prod-Audit requires 7-year audit log retention with immutability, Test-Audit requires 1-year retention, Archive-Audit is read-only for compliance reporting only (no active operations). Additionally, new subscriptions added to the management group must auto-inherit the audit team's Reader role, but ONLY for Prod-Audit and Test-Audit (not Archive-Audit, which should require explicit approval). The assignment must support auto-audit-logging of audit-team read operations for SOX compliance, and failed audit read attempts must trigger alerts to the security team. At which scope(s) should you assign the Reader role, and which additional layering is required to satisfy all requirements?",
    stem: "Which role assignment scope and configuration satisfy inherited read access, compliance-specific retention, selective auto-inheritance, and audit-triggered alerting?",
    subtopic: "Manage access to Azure resources",
    referenceTopic: "Scope access assignments, RBAC inheritance, Diagnostic Settings for audit logging, alert rules",

    hint: "If you assign Reader at the Audit management group, it flows to ALL children including Archive-Audit (unwanted). What if you assign at Prod-Audit and Test-Audit separately? How do you auto-inherit on NEW subscriptions without including Archive-Audit? Can PIM approvals gate Archive-Audit additions? How do Diagnostic Settings + Alert Rules let you audit read-operations and trigger security alerts on failures?",
    options: [
      option("A", "Assign Reader at the Audit management group; new subscriptions inherit automatically.", "Incorrect. This assigns Reader to ALL child subscriptions including Archive-Audit (which should require explicit approval). The scenario mandates selective auto-inheritance."),
      option("B", "Assign Reader at Prod-Audit and Test-Audit management group scopes directly. Enable Diagnostic Settings on each subscription to log audit-team read operations. Create alert rules to notify security on failed read attempts. For future subscriptions, use a Policy assignment that auto-assigns Reader only if the subscription name contains 'Prod' or 'Test'.", "Correct. Direct assignment at child scopes (Prod-Audit, Test-Audit) allows selective inheritance; Archive-Audit remains excluded unless explicitly approved. Diagnostic Settings capture read-operations for audit compliance. Alert rules provide security notification on failures. Policy-based naming convention auto-assigns future subscriptions based on naming pattern without affecting Archive-Audit."),
      option("C", "Assign Reader at the Audit management group. Use Azure Policy with Deny effect to block Archive-Audit access for auditors.", "Incorrect. Deny policies block ACTIONS (preventing read operations), not ROLE ASSIGNMENTS. Auditors would still have the Reader role on Archive-Audit; the policy would just deny their actual reads, creating a contradiction. The correct approach is to NOT assign the role in the first place."),
      option("D", "Assign Reader at each resource group within each subscription, and require manual updates for each new resource group.", "Incorrect. Resource-group-level assignments do not provide subscription-wide inherited access. Scaling to new subscriptions would require manual effort for every new subscription and every resource group within it, violating the auto-inheritance requirement."),
    ],
    correctOptionId: "B",
    explanation:
      "This tests deep understanding of role scope selection, selective inheritance patterns, and how to layer RBAC with policy and monitoring for compliance. Assigning Reader at child scopes (Prod-Audit, Test-Audit) provides inheritance within those subtrees while excluding Archive-Audit. Diagnostic Settings + Alert Rules satisfy audit-logging and security notification requirements. Policy-based auto-assignment on new subscriptions avoids manual work while respecting the naming-based differentiation. The scenario challenges the assumption that 'management group assignment = simplest solution' by introducing selective inheritance and compliance-specific layering.",
  }),
  choiceQuestion({
    id: "Q2106",
    domain: "D1",
  ...
  choiceQuestion({
    ...
    scenario:
      "Nod Publishers operates multiple subscriptions for development, staging, and production. Monthly budget is $50K. Finance requires: (1) Cost alerts at 80% ($40K) and 100% ($50K) thresholds via email to finance team + Slack to ops team, (2) Alerts must differentiate between forecasted overage (estimate before month-end) vs actual overage (already spent), (3) Production subscription must have a 'critical incident' exemption allowing ops to provision resources without cost gates during P1 incidents, but must auto-log all emergency spend to audit trail, (4) Finance must auto-remediate non-production overspends by stopping resource deployments (but NOT stopping running workloads), (5) Monthly spend must be reconciled across all subscriptions with per-resource-group breakdowns, (6) During a 30-day change-freeze (Dec 1-30), any new budget increases must require CISO + CFO dual approval, and must NOT trigger auto-remediation even if forecasted spend exceeds threshold. Which combination of services and configurations satisfies all six requirements?",
    stem: "Which integrated cost-control and governance setup provides threshold alerting, incident exemptions with audit logging, auto-remediation for non-prod, reconciliation, and change-freeze compliance?",
    subtopic: "Manage Azure subscriptions and governance",
    referenceTopic: "Azure Cost Management, Budget Alerts, Policy-based remediation, Audit logging, Change management integration",

    hint: "Budgets provide alerting (requirement 1-2). But how do you auto-remediate overspend (requirement 4)? Budgets don't block deployments; you need Azure Policy. How do you grant critical-incident exemptions (requirement 3)? Can a Policy Modify or Deny effect be time-gated or incident-gated? What audit trail captures emergency exemptions? For requirement 6 (change-freeze approval), how do you gate BUDGET CHANGES (not resources) with dual-approval? How do you integrate incident-management systems (PagerDuty, etc.) with Policy or Budgets for incident exemption validation?",
    options: [
      option("A", "Configure budget with 80% and 100% alert thresholds. Create Alert Action Groups that email finance and post to Slack. This satisfies the requirements.", "Incorrect. Budgets provide alerting (requirements 1-2) but do NOT auto-remediate overspend (requirement 4), do NOT log emergency exemptions (requirement 3), and cannot gate budget changes with approval workflows (requirement 6). This is incomplete."),
      option("B", "Configure budget with forecast + actual alerts to two Action Groups (email + Slack). Layer an Azure Policy with Deny effect that blocks Microsoft.Compute/virtualMachines creation when budget threshold is exceeded. For critical incidents, update the Policy assignment's Managed Identity permissions via a PagerDuty webhook so incidents auto-trigger exemption. Use Diagnostic Settings to log all Policy denials. For change-freeze compliance, implement a custom Azure Function that gates budget changes and requires dual approval (CISO + CFO).", "Correct. This layers multiple services: Budget for alerting (1, 2), Policy Deny for auto-remediation (4), incident-integrated exemption via webhook + Managed Identity (3 with audit trail), and custom Function for change-freeze dual-approval gates (6). Diagnostic Settings satisfy audit logging for compliance. The multi-layer approach recognizes that NO single service solves all requirements; you must combine Budget (alerting), Policy (remediation), incident-management integration (exemptions), and custom orchestration (change-freeze approval)."),
      option("C", "Configure budget with threshold alerts and create a custom Policy that auto-denies deployments when forecasted spend exceeds 80%. For exemptions, add specific user IDs to the Policy exclusion list permanently during Q4 (Dec 1-30).", "Incorrect. Permanent user exemptions bypass compliance controls entirely and violate change-freeze policy. You cannot selectively exclude users during specific time windows without a time-gated or incident-triggered mechanism."),
      option("D", "Use Azure Cost Analysis with manual approval gates and require the CFO to approve all deployments exceeding $1K in the production subscription.", "Incorrect. Manual approval gates are not automated (require human sign-off for every deployment) and do not satisfy the requirement for auto-remediation. Manual gating also violates the 'allow emergency override' requirement by creating operational friction."),
    ],
    correctOptionId: "B",
    explanation:
      "This tests understanding of cost governance as a MULTI-LAYER system: Budget for alerting/forecasting, Policy for auto-remediation, incident-management integration for emergency exemptions with audit trails, and custom orchestration for approval workflows. Option A assumes Budgets alone are sufficient (they're not). Option C uses permanent exclusions instead of incident-gated exemptions. Option D over-burdens finance with manual approvals. The correct answer demonstrates that cost control + compliance + incident flexibility requires layering Budget + Policy + integration points + custom automation to achieve all six requirements simultaneously.",
  }),
  choiceQuestion({
    id: "Q2111",
    domain: "D1",
  ...
  choiceQuestion({
    ...
    scenario:
      "Fourth Coffee uses a shared operations subscription with strict separation of duties. A platform lead must grant and revoke RBAC assignments for app teams across multiple resource groups, including emergency break-glass assignments during incidents. The same user must NOT be able to create, update, restart, or delete any compute, network, or storage resources. Security also requires this role to be reusable at management group and subscription scopes without custom role authoring.",
    stem: "Which built-in role best satisfies these requirements with least privilege?",
    subtopic: "Manage access to Azure resources",
    referenceTopic: "Built-in Azure RBAC roles",

    hint: "Separate permission planes: data/control operations on resources versus access-control operations on role assignments. Which built-in role grants access management actions but avoids broad resource management rights?",
    options: [
      option("A", "Reader", "Incorrect. Reader can view resources but cannot create or remove role assignments, so it fails the access-administration requirement."),
      option("B", "Contributor", "Incorrect. Contributor can create and modify most resources, violating the separation-of-duties requirement that forbids resource operations."),
      option("C", "Virtual Machine Contributor", "Incorrect. This role is workload-specific and does not provide broad RBAC assignment administration across non-VM resources."),
      option("D", "User Access Administrator", "Correct. User Access Administrator can manage role assignments while not granting broad create/modify/delete rights over resources themselves."),
    ],
    correctOptionId: "D",
    explanation:
      "User Access Administrator is the correct least-privileged built-in role for RBAC delegation scenarios where a user must manage assignments but must not perform general resource lifecycle operations. Reader lacks assignment permissions, while Contributor and workload-specific contributor roles exceed required privileges. This question tests separation of duties between access governance and resource administration.",
  }),
  multiSelectQuestion({
    id: "Q2113",
    domain: "D1",
  ...
  multiSelectQuestion({
    ...
    scenario:
      "Northwind Health is removing standing privileged access for subscription administrators after an audit finding. Admins must request elevation only when needed, each request must include justification, activation must require approver consent, and emergency incidents must still allow activation within minutes after approval. Access reviews are already configured separately and should not be relied on for real-time gating.",
    stem: "Which TWO PIM settings directly enforce just-in-time elevation with approval-gated activation?",
    subtopic: "Manage access to Azure resources",
    referenceTopic: "Microsoft Entra Privileged Identity Management",

    hint: "Focus on activation-time controls, not periodic governance controls. Which settings remove always-on privilege and then gate activation before role use?",
    options: [
      option("A", "Configure role assignment as Eligible", "Correct. Eligible assignments remove standing privilege and force on-demand activation."),
      option("B", "Require approval for activation", "Correct. Approval adds a real-time control gate before privileged access becomes active."),
      option("C", "Enable access reviews only", "Incorrect. Access reviews are periodic governance checks, not activation-time enforcement for each request."),
      option("D", "Apply a CanNotDelete lock", "Incorrect. Locks protect resource operations but do not govern PIM activation workflows."),
    ],
    selectCount: 2,
    correctOptionIds: ["A", "B"],
    explanation:
      "The required controls are PIM Eligible assignments (removes standing access) plus approval-required activation (real-time gate). Access reviews are useful for periodic cleanup but do not enforce per-activation approval. Resource locks are unrelated to identity elevation. This tests understanding of PIM operational controls versus adjacent governance features.",
  }),
  multiSelectQuestion({
    id: "Q2115",
    domain: "D1",
  ...
  choiceQuestion({
    ...
    scenario:
      "Contoso has a management group hierarchy: Root > Corp > Prod (with sub-management groups Prod-Core, Prod-StagingTest) and Dev. A network security policy must apply to all Prod-Core production subscriptions, AND a separate less-strict compliance policy must apply to Prod-StagingTest (which hosts pre-release workloads). Dev subscriptions must NEVER receive either policy. Additionally, if any production subscription is mistakenly moved from Prod to Root, the policies must NOT follow it (to prevent policy inheritance outside the intended scope). Auditors must verify that policy assignments do NOT apply outside their intended management group boundary.",
    stem: "At which scope(s) should you assign these policies so Prod-Core gets the strict policy, Prod-StagingTest gets the less-strict version, Dev gets neither, and accidental subscription migrations do NOT trigger unwanted policy inheritance?",
    subtopic: "Manage Azure subscriptions and governance",
    referenceTopic: "Policy scopes, management group hierarchy, and policy containment",

    hint: "Policy inheritance follows management group hierarchy downward ONLY if assigned at a parent scope. What happens if a policy is assigned at a CHILD scope (vs parent)? Can you prevent a policy from flowing up or across sibling management groups? Why does assignment DEPTH matter? Consider: Assign the strict policy at Prod-Core scope directly—does it affect Prod-StagingTest? What if you assign at Prod scope (their parent)? How do accidental migrations interact with scope containment?",
    options: [
      option("A", "Assign the strict policy at the Prod scope so it flows down to both Prod-Core and Prod-StagingTest. Then assign the less-strict policy at Prod-StagingTest to override it.", "Incorrect. Policies do not override; multiple policies are ADDITIVE. Both policies would apply to Prod-StagingTest, not just the less-strict one. Also, this violates the requirement that Prod-StagingTest receive only the less-strict version."),
      option("B", "Assign the strict policy at the Prod-Core management group scope directly. Assign the less-strict policy at Prod-StagingTest management group scope directly. This ensures each policy applies ONLY within its intended child management group and does NOT inherit upward or sideway.", "Correct. Policies assigned at a child management group scope apply to that subtree only and do NOT flow to siblings or parents. If a Prod-Core subscription is mistakenly moved to Root, it no longer inherits the Prod-Core policy because the policy was assigned at the Prod-Core scope, not at a parent scope. This containment prevents unwanted inheritance during migrations. Auditors can verify by checking assignment scopes directly."),
      option("C", "Assign both policies at the Root management group scope with exemptions for Dev. This centralizes policy management at the root.", "Incorrect. Assigning at Root means policies flow down to ALL child management groups, including Dev. Exemptions are supported but add operational complexity. The scenario asks for scope containment that naturally prevents Dev from receiving any policies."),
      option("D", "Assign the strict policy at the Prod scope and use a Deny policy at Dev to block it. Assign the less-strict policy at Prod-StagingTest.", "Incorrect. Deny policies do not prevent inheritance; they prevent ACTIONS. A Deny policy cannot block a parent policy assignment from applying. The correct approach is to assign policies at the CHILD scopes, not parent scopes, to contain them."),
    ],
    correctOptionId: "B",
    explanation:
      "Assigning policies at child management group scopes (Prod-Core and Prod-StagingTest directly) ensures they apply ONLY to their intended subtrees and do NOT flow upward to parents or sideways to siblings. This containment prevents accidental inclusion if a subscription is migrated outside the intended hierarchy. Policies assigned at parent scopes inherit downward to all children; policies assigned at child scopes stay local. This tests understanding of policy scope DEPTH and hierarchy containment, the asymmetry of inheritance (down but not up), and why CHILD-LEVEL assignments are necessary for compliance boundaries that must withstand structural changes.",
  }),
  choiceQuestion({
    id: "Q2122",
    domain: "D1",
